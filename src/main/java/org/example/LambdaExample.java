...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<User> isActive = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.active;
            }
        };
        Comparator<User> sortByName2 = new Comparator<User>() {
            @Override
            public int compare(User u1, User u2) {
                return u1.name.compareTo(u2.name);
            }
        };
        repository.select(isActive, sortByName2);
        ...
        Comparator<User> sortByName3 = new Comparator<User>() {
            @Override
            public int compare(User u1, User u2) {
                return u1.name.compareTo(u2.name);
            }
        };
        repository.select(activeAndAgeGreaterThan8, sortByName3);
